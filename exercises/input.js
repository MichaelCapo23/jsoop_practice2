class Input {
    ...
    constructor(tagetElement) {
        this.test = this.test.bind(this);
        this.testObj = {
            targetElelemt: tagetElement,
            pattern: null,
            elementMessage: null,
            error : null
        }
    ...
    setRange(minNum, maxNum) {
        this.minMaxObj.min = minNum;
        this.minMaxObj.max = maxNum;
    }
    ...
    test() {
        this.regexP = this.testObj.pattern;
        this.targetInput = this.testObj.targetElelemt;
        var input = this.targetInput.val();
        var result = this.regexP.test(this.targetInput.val());
        if (result) {
            this.testObj.result = {
                "result": result
            }
        ...
        if (this.minMaxObj.max !== null && this.minMaxObj.min !== null) {
            this.min = this.minMaxObj.min;
            this.max = this.minMaxObj.max;
            if (input > 10 || input < 1) {
                this.testObj.result = {
                    result: false,
                    error : "range"
    ...
    notes: should create an element and position it directly beneath the input
        ...
        */
    showError(message) {
        this.elementPosition = this.testObj.targetElelemt.position();
        this.elementHeight = this.testObj.targetElelemt.css("height");
        var newDomElement = $("<div>").css("position", "absolute").css("left", this.elementPosition.left).css("top", this.elementPosition.top + 20).addClass("inputError").text(message)
        this.testObj.targetElelemt.parent().append(newDomElement);
        this.testObj.error = newDomElement;
    }
